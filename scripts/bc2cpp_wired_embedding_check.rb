...
# point of that class is installed by the gem's hand-written register.cxx. An
# unregistered one keeps its interpreted body, which reads the ordinary ivar
# table while the compiled methods write the struct, and sees nil (in the
# desktop build: `nil >= x` in Scene::Map#step_parallel, swallowed by a rescue).
#
# Runs the real generator once per compiled gem that owns a wired class and
# compares its "== compiled entry points ==" listing with that gem's
# register.cxx. Needs a host mrbc (MRBC), like the other bc2cpp checks; takes
# about as long as bc2cpp_coverage_report.rb per gem.
...
BC2CPP_COMPILED_GEMS.each do |name, gem|
  ...
  Dir.mktmpdir do |dir|
    ...
    _out, err, status = Open3.capture3(env, RbConfig.ruby, "#{root}/tools/bc2cpp/bc2cpp.rb", *srcs)
    ...
    identifiers = register.scan(/\b[A-Za-z_][A-Za-z0-9_]*\b/).to_set
    wired.each do |owner|
...
if failures.empty?
  ...
  warn "bc2cpp wired embedding check: #{failures.size} class(es) embedded without complete registration: " \
       "#{failures.join(', ')} -- register the missing entry points in register.cxx or drop the class from " \
       'BC2CPP_WIRED_EMBEDDINGS'
  exit 1
end
