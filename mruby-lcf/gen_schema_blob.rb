...
class Generator
  ...
  def encode_field_body(w, field)
    # `field` is a real LCF::Schema::FieldSchema now (schema.rb's own
    # Hash-literal-to-Struct conversion), not a Hash -- `#[]` still works
    # identically (mruby's own Struct supports Symbol-keyed bracket access,
    # confirmed against 3rd/mruby/mrbgems/mruby-struct/src/struct.c), but
    # `#fetch`/`#key?` don't exist on Struct at all (CRuby's own Struct
    # doesn't define them either -- confirmed directly, this script runs
    # under CRuby). `:name`/`:type` are mandatory on every real entry
    # (confirmed: 932 of 932 schema.rb entries set both), so a bare `[]`
    # read is exactly as safe as the `.fetch` it replaces.
    name_idx = string_index(field[:name])
    type_tag = TYPE_TAGS.fetch(field[:type]) { raise "unknown type: #{field[:type]}" }
    has_order = !field[:order].nil?
  ...
  def generate
    ...
    LCF::Schema.constants.sort.each do |cname|
      ...
      # FieldSchema itself (schema.rb's own Hash-to-Struct conversion) is
      # now a real constant of LCF::Schema too, alongside the actual
      # schema data -- not itself schema data, so skip it the same way
      # SCROLL_UNITS_PER_PIXEL already is. A real, live bug caught running
      # this generator for real (not just `ruby -c`): `val.is_a?(Class)`
      # is `true` here for the FieldSchema class object itself (`cname ==
      # :FieldSchema`), which none of the branches below correctly handle
      # -- it previously fell through to the final `else` (`section_index`
      # expects an id-keyed Hash to call `#sort_by` on, which a Class
      # object doesn't have), a real `NoMethodError` at generation time.
      ...
      elsif val.is_a?(LCF::Schema::FieldSchema)
        # A real class check now (schema.rb's own Hash-to-Struct
        # conversion gave every field record a concrete, distinct type) --
        # used to be `val.key?(:name) && val.key?(:type)`, a duck-typed
        # stand-in for "this is a field record, not a section" back when
        # both shapes were plain Hashes; strictly more precise now, not
        # just mechanically translated (a section Hash could in principle
        # have coincidentally held keys called `:name`/`:type` of its own
        # -- never happened in practice, but this check no longer has to
        # rely on that).
        #
...
# Same two exclusions as the `generate` method's own constant walk above
# (`Integer`: SCROLL_UNITS_PER_PIXEL; `Class`: FieldSchema itself) -- this
# is a SEPARATE walk over the same `LCF::Schema.constants` list (building
# the generated decoder's own `NAME = Blob.top(:NAME)` assignment lines,
# not the blob's own byte content), so it needs the identical filter or a
# real, live bug results: confirmed by running this generator for real
# after schema.rb's own Hash-to-Struct conversion -- omitting the `Class`
# exclusion here left a stray `FieldSchema = Blob.top(:FieldSchema)` line
# in the generated output (`Blob.top` has no `:FieldSchema` entry to
# return, since the byte-content walk correctly never encoded one), a
# real generated-file diff caught by comparing this generator's own output
# byte-for-byte against its pre-conversion baseline.
top_names = LCF::Schema.constants.sort.reject do |c|
  val = LCF::Schema.const_get(c)
  val.is_a?(Integer) || val.is_a?(Class)
end
