...
- 🚧 Event command interpreter — `Game::Interpreter` runs a solid subset (Show
  ...
  Level** and **Change Equipment** apply to a fixed actor, a variable-selected
  actor or the whole party, clamped to each actor's maxima (Change HP honours the
  allow-death floor; Change Parameters re-clamps current HP/MP when a maximum is
  lowered; Change Equipment folds an equipped item's bonuses into the effective
  stats). **Change EXP** and **Change Level** grow (or shrink) an actor: EXP
  re-derives the level from the RPG2000 standard curve
  ...
  not refilled, matching RPG_RT). **Control Variables** reads not just constants
  and other variables but also a **random** range, an **actor stat** (level /
