// LVGL configuration for the Maix Amigo firmware.
//
// Same embedded-tuned shape as app/wio/lv_conf.h: CLIB malloc (one shared
// newlib heap with mruby instead of a second static pool), the canvas/image/
// label-only widget set the RGSS render path needs, no themes/layouts. The
// trims' justification there (single-format RPG2k-only Ruby, same sources
// here) holds verbatim, so they are inherited rather than re-measured; the
// K210's 8 MB SRAM could hold the full set, but dead code is dead code.
// LVGL v9 fills every option not set here with the default from
// lv_conf_internal.h, so this only overrides what the board needs.
//
...
/* On, unlike wio/psp (which keep 0 with their trims): this port compiles the
 * untrimmed lib.cxx, which reaches lv_log_add through LVGL's own static
 * inlines (lv_obj_move_foreground and friends) -- with LOG off lv_log.c
 * compiles the function out and the firmware link fails on it, the same
 * mismatch mruby-rgss/mrbgem.rake's own comment documents for
 * LV_USE_SNAPSHOT. Desktop parity, not a new need. */
#define LV_USE_LOG 1
