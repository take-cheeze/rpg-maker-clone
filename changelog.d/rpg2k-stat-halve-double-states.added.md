- RPG Maker 2000: states can now **halve or double a battler's stats**. A
  ...
  `#effective_agi` (ported from a reference implementation, minus its
  battle-only stat-`mod` term, not independently confirmed against genuine
  RPG_RT under wine) feed basic-attack and self-destruct damage,
  ...
  stat on the same battler) net to no change, matching that reference
  implementation's own halve-vs-double guard. A battle **Skill**'s power formula
  ...
  just in-battle skills, matching how that reference implementation routes
  every context through one accessor for these stats. Covered by new
