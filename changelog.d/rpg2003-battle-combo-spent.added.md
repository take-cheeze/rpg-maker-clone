- The **RPG2003 battle combo is now spent** — the one Enable Combo (1007)
  ...
  mapping to a reference implementation's default ids 1-4, a customized list
  to its own refs),
  ...
  real RPG_RT) a Defend / Item / Escape command is never combo'd. The
  combo stays armed for the whole fight, matching that reference
  implementation's combo-processing logic — ported from it, not
  independently confirmed against genuine RPG_RT under wine. Covered by new `rpg2k_logic_check.rb` checks
