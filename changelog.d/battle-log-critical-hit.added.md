- **The battle log announces a critical hit in the game's own words.**
  ...
  test beds' data alone. Reading a reference implementation's own
  critical-hit-message logic (not independently confirmed against genuine
  RPG_RT under wine) settles it: the term is keyed on the **target** taking the crit, the same
