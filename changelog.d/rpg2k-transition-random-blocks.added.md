- **Erase / Show Screen's random-blocks styles (settings 1–3) now paint real
  ...
  styles were already done). Ported from a reference implementation's real
  source, not independently confirmed against genuine RPG_RT under wine: a
  4×4-pixel block (so 320×240 is an 80×60 grid, 4800 blocks) and a linear
  cumulative-reveal ramp — 120 blocks a frame over the 41-frame default
  length. `Game::Transition#new_block_rects` returns only the
  ...
  attempting to replicate a reference implementation's own windowed per-row
  shuffle, since that depends on matching its RNG stream too. Covered by new checks in
