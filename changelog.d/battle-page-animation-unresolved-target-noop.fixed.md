- **Show Battle Animation (battle-event page form):** an out-of-range Ally
  ...
  Matches RPG_RT's own Show Battle Animation command handling, ported from a
  reference implementation, not independently confirmed against genuine
  RPG_RT under wine, which no-ops the entire command whenever the named
  party/troop slot doesn't exist. A "wait until it finishes" request on such a target now
