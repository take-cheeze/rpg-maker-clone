- **A common event's own Parallel Process now advances before any map
  ...
  *following* frame. Verified against a reference implementation's source,
  not independently confirmed against genuine RPG_RT under wine: common
  events are always updated before map events, with no interleaving by id
