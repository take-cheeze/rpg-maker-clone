- RPG Maker 2000: **basic attacks can now miss**. `Game::Battle#to_hit` computes
  ...
  adjusted by the attacker/target agility ratio, matching a reference
  implementation's to-hit formula (which reduces to
  `100 - (100 - base)·(srcAgi + tgtAgi)/(2·srcAgi)`, clamped 0..100), not
  independently confirmed against genuine RPG_RT under wine, so a nimbler
