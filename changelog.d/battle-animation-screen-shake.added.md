- **A Battle Animation's per-frame `screen_shaking` timing now actually
  ...
  sibling `flash_scope` field — never read it. Ported from a reference
  implementation's animation-timing processor, not independently confirmed
  against genuine RPG_RT under wine: both cases fire a fixed
  ...
  mirroring a reference implementation's own battle-target shake code (again
  not independently confirmed against genuine RPG_RT under wine); a
  map-triggered Show Battle Animation's target case is left a
  genuine no-op, matching that same source's own empty
  method body for the map-triggered case rather than treating it as an
  oversight to fill in. Covered by six new `scripts/rpg2k_scene_check.rb` checks (screen
