- **Message text blended with the windowskin** — `\c[n]` message colours now
  ...
  160×80 System image, per EasyRPG's layout) and `Scene::Map` blends each run
  against it, falling back to a flat colour only when no windowskin loaded or for
  an out-of-range index. Swatch geometry is pinned by
  `scripts/rpg2k_render_check.rb` and the render path (blend vs flat fallback) by
