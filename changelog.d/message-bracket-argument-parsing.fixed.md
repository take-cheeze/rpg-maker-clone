- **RPG2000/2003 messages:** `\N[]`/`\V[]`/`\C[]`/`\S[]` bracket arguments now
  parse character-by-character like RPG_RT's own bracket-argument parsing,
  ported from a reference implementation, not independently confirmed
  against genuine RPG_RT under wine, instead of extracting the raw
  substring up to the first `]` and re-parsing it. A literal digit run and a nested `\V[]` reference in the same bracket
