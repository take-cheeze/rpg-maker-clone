- **CI: drop the ~20k-line git ref dump nix prints before every build.** Because
  ...
  rather than logging through it. New `scripts/drop-git-ref-noise.bash` filters
  those lines out of `scripts/nix-develop.bash` and the flake job's
  `nix build -L`, leaving `From <url>`, `remote:` counters and any fetch failure
  visible.
