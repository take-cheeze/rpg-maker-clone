- **A battle/vehicle/inn BGM matching the file already playing no longer
  ...
  though this scene never actually stopped it. Ported from a reference
  implementation's own single native BGM-play entry point, not independently
  confirmed against genuine RPG_RT under wine, that every one of these
  transitions funnels through in the real engine, including battle entry.
  Fixed by extracting a shared
