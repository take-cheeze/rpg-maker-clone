- **A map event page's RPG2003-only Timer2 condition is now consulted, and
  ...
  instead of always testing `>=`.** Ported from a reference implementation,
  not independently confirmed against genuine RPG_RT under wine: RPG2003
  lets a page condition compare a variable with `==`, `<=`,
