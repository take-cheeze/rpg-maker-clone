- **`--rgss_host_move_test` walks the party in a game's own engine.** Once the
  ...
  `[RPGXP-HOST-MOVE] start=.. end=.. moved=.. frame=..`, read from the game's own
  ...
  asserted. Both markers carry the frame they were reported on, which is what
  separates a game that stalled from a headless run whose frames were simply
  slower than its wall-clock budget.
