- **An upper-layer chipset tile now only draws in front of the hero/events
  ...
  RPG_RT reads the same `ABOVE_BIT` this renderer already used for
  ...
  the hero across a 3-tile bed graphic (headboard / mattress / footer); only
  the footer is starred, so the headboard and mattress alone would show him
  in full, but a separate above-hero map event supplies the small pillow
  graphic that covers him from the neck down, and his head now clears the
  pillow the way RPG_RT's does. Treating every upper tile as always-above
  left him fully invisible instead of tucked in. New
