- **A batch (range) Control Variables write whose own operand reads a
  ...
  instead of broadcasting one value to the whole range.** Ported from a
  reference implementation, not independently confirmed against genuine
  RPG_RT under wine: its own range-variable write, reached whenever
  its Control Variables command handler
  ...
  `#do_control_vars_range_variable`, ported from that same reference
  implementation's two-pass split; a source outside the destination range still degenerates
