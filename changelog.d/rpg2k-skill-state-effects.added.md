- RPG Maker 2000: **field skills now change status conditions**. `cast_skill`
  ...
  matching a reference implementation's approach here — ported, not
  independently confirmed against genuine RPG_RT under wine — and states are
  applied **before**
  ...
  spends SP when it actually changed something. Grounded in a reference
  implementation's field skill handling (scope self/ally/party,
  `reverse_state_effect` polarity, no to-hit) — ported, not independently
  confirmed against genuine RPG_RT under wine. Covered by new
  `scripts/rpg2k_logic_check.rb` checks (cure only the
