...
# ---- Wolf::Crypt v3.5 Pro-protected decryption (wolf_crypt_pro.rb) ---------
#
# Every expected value below was recorded from a small standalone C++
# harness compiling WolfTL's own vendored headers
# (/home/user/3rdref/WolfTL/WolfTL/WolfCrypt/*.hpp) unmodified -- not
# hand-traced from reading them. See
# docs/adr/0095-wolf-rpg-editor-pro-protected.md for the harness source and
# how to reproduce it. scripts/wolf_pro_protected_check.rb is the much
# stronger cross-check on top of these unit vectors: a full v3.5-protected
# round trip through Wolf::Project, over the real downloaded sample game,
# the way scripts/wolf_data_wolf_check.rb already does for Wolf::DataWolf.

assert "Wolf::Crypt::Sha512 matches the compiled WolfTL harness for three inputs" do
  dyn_salt = [1, 2, 3, 4]
  salted = Wolf::Crypt::Sha512.salt_password([], dyn_salt, "basicD1".bytes)
  assert_equal(
    "680e7a7f1b855303e6a437f84192eab84a12f95082fd8e5a6174b6fc34a4f34" \
    "aa3d4d00ce9626010f57680bf3e6e0ea1e644723e45c6c44537e51a28ba9494" \
    "98",
    Wolf::Crypt::Sha512.hexdigest(salted)
  )
  assert_equal(
    "02793e148ba17d7e5b1f9d0836b08efd1dcc2d33047748f9184f56ecb5132d2" \
    "955df53a26ad2c63190df383ed561d617f8faac3e9fd68f9574928b82c660e5" \
    "a9",
    Wolf::Crypt::Sha512.hexdigest("abc".bytes)
  )
  assert_equal(
    "25940edc34369c2b86890e8cb53eee5f796ff3a7ef6ec1841ba5a43176ef310" \
    "37735d6f846aa6b86f15b4c2426030fbb97a4447f97f6c31366ad76bba9f419" \
    "6e",
    Wolf::Crypt::Sha512.hexdigest([])
  )
end

assert "Wolf::Crypt::Sha512.calc_dyn_salt matches the compiled WolfTL harness" do
  data = Array.new(20) { |i| (i * 7 + 3) & 0xFF }.pack("C*")
  assert_equal [0xD4, 0x3B, 0x63, 0x42], Wolf::Crypt::Sha512.calc_dyn_salt(data)
end

assert "Wolf::Crypt::Aes key expansion and CTR keystream match the compiled WolfTL harness" do
  key = Array.new(16) { |i| (i * 3 + 1) & 0xFF }
  iv = Array.new(16) { |i| (0xA0 + i) & 0xFF }
  round_key = Wolf::Crypt::Aes.key_expansion(key) + iv
  assert_equal(
    "0104070a0d101316191c1f2225282b2e340bc974391bda622007c540052fee" \
    "6e2309a9a21a1273c03a15b6803f3a58eea70f7e48bd1d0d888708bb08b832e" \
    "3e68c0e0f9031130218b61bb9100e295af63905b2c70816b0dfbe0d09cfb024" \
    "53392f0b5f08271defd79910e6182934b52177065d43501bb294c90b548ce0" \
    "3fe1ad82093780d21285141b19d198fb2630356e095e9ebc1bdb8aa7020a125" \
    "c243a276e016d0ad21ab6807518bc92293c86b5a0a1a2a3a4a5a6a7a8a9aaab" \
    "acadaeaf",
    round_key.map { |b| sprintf("%02x", b) }.join
  )

  data = (0...40).to_a
  cipher = data.dup
  Wolf::Crypt::Aes.ctr_xcrypt!(cipher, round_key)
  assert_equal(
    "3aaa552e110074160655ee86fbc4f408cf04344ef428dd092f8d5b2faf4942f" \
    "be3fc5de78cd7dce9",
    cipher.map { |b| sprintf("%02x", b) }.join
  )

  # CTR is symmetric: encrypting again with a freshly-derived, identically-
  # keyed round key (the IV is mutated in place, so a fresh copy is needed)
  # recovers the original plaintext.
  round_key2 = Wolf::Crypt::Aes.key_expansion(key) + iv
  Wolf::Crypt::Aes.ctr_xcrypt!(cipher, round_key2)
  assert_equal data, cipher
end

assert "Wolf::Crypt.xorshift32_step matches the compiled WolfTL harness" do
  s1 = Wolf::Crypt.xorshift32_step(0xDEADBEEF)
  s2 = Wolf::Crypt.xorshift32_step(s1)
  s3 = Wolf::Crypt.xorshift32_step(s2)
  assert_equal 0x1E329284, s1
  assert_equal 0xD9F74BD0, s2
  assert_equal 0xB74A1525, s3
end

assert "Wolf::Crypt.msvc_rand_next matches the compiled WolfTL harness" do
  expected = [7584, 19164, 25795, 22125, 5828]
  state = 12345
  expected.each do |want|
    r, state = Wolf::Crypt.msvc_rand_next(state)
    assert_equal want, r
  end
end

# `decrypt_v35`/`encrypt_v35`/`decrypt_protected` all take/return byte
# Strings, not Arrays -- a real protected file (CommonEvent.dat especially)
# can be several hundred KB, well past mruby's `MRB_ARY_LENGTH_MAX` (131072)
# Array-length cap; see wolf_crypt_pro.rb's own portability note. `bytes_of`
# builds the small synthetic test buffers below directly as Strings (via
# Array#pack) so no test here ever depends on Array semantics for the big
# buffer either.
def bytes_of(size, mult, add)
  # Built in bounded chunks, never one `size`-length Array -- this helper's
  # own job is building the >131072-byte buffer the "past mruby's Array
  # length cap" test below needs, so it has to stay under the cap itself
  # (the same CHUNK discipline data_wolf.rb's own fixture-building code
  # uses, mruby-wolf/mrblib/data_wolf.rb's own `CHUNK`).
  chunk_size = 65536
  parts = []
  i = 0
  while i < size
    n = [chunk_size, size - i].min
    parts << Array.new(n) { |j| ((i + j) * mult + add) & 0xFF }.pack("C*")
    i += n
  end
  parts.join
end

assert "Wolf::Crypt.decrypt_v35 matches the compiled WolfTL harness for all four known file types" do
  buf = bytes_of(200, 13, 7)
  buf.setbyte(1, 0x50)
  buf.setbyte(5, 0x57)

  out = Wolf::Crypt.decrypt_v35(buf, "GameDat", Wolf::Crypt::FileType::GAME_DAT)
  assert_equal 67, out.bytesize
  assert_equal "\x00\x57\x00\x00\x4F\x4C\x00\x46\x4D\x55".b, out.byteslice(0, 10)
  assert_equal(
    "005700004f4c00464d552e0ca8edd03d8f778e88e5169abcc1717af5dc9b97" \
    "c7b3630e7141014963f670fc7f945fee5a014c3855d760e0350d588546c036" \
    "6418e7338c",
    out.bytes.map { |b| sprintf("%02x", b) }.join
  )

  out = Wolf::Crypt.decrypt_v35(buf, "CommonEvent", Wolf::Crypt::FileType::COMMON_EVENT)
  assert_equal(
    "005700004f4c55464300dd710bbe6e33d4cf713604aaa28ac4087ab71018d04" \
    "41cc3c51980818f295ba7b44c1dff88ac90c9dfb5e1de66cd6f524f0c4fe36" \
    "1c7d8ea30",
    out.bytes.map { |b| sprintf("%02x", b) }.join
  )

  out = Wolf::Crypt.decrypt_v35(buf, "DataBase", Wolf::Crypt::FileType::DATA_BASE)
  assert_equal(
    "005700004f4c55464d00ebc842b3686d9f76b8c0e1dfde6b65c232b5aafe23" \
    "1837ad9a0554311b963b07b22cafbcbb0171ca5c8529a3432d500320b5ec8f" \
    "c1b2707128",
    out.bytes.map { |b| sprintf("%02x", b) }.join
  )

  out = Wolf::Crypt.decrypt_v35(buf, "TileSetData", Wolf::Crypt::FileType::TILE_SET_DATA)
  assert_equal(
    "005700004f4c55464d001528fccb9401e9e67e67c12e3489d0de2be4d30692" \
    "131adf22f266a7625046480b0ba86ad17fd847a9a5f08c242e282ed12d0712" \
    "2868c37db6",
    out.bytes.map { |b| sprintf("%02x", b) }.join
  )
end

assert "Wolf::Crypt.decrypt_v35 matches the compiled WolfTL harness on a buffer large enough to trigger the aesSize cap" do
  buf = bytes_of(2000, 17, 11)
  buf.setbyte(1, 0x50)
  buf.setbyte(5, 0x57)
  out = Wolf::Crypt.decrypt_v35(buf, "GameDat", Wolf::Crypt::FileType::GAME_DAT)
  assert_equal 1867, out.bytesize
  # Full-buffer digest would be unwieldy in a test source file; the region
  # the aesSize cap actually protects (bytes just past the discarded
  # 143-byte prefix, i.e. the start of the real body) is enough to catch a
  # wrong cap computation, and scripts/wolf_pro_protected_check.rb already
  # covers the buffer end to end against a real project.
  assert_equal(
    "362a07b17e4357efbb582246f0c48f6914a0530d0a342e63067cde3d00479ec825",
    out.byteslice(10, 33).bytes.map { |b| sprintf("%02x", b) }.join
  )
end

assert "Wolf::Crypt.decrypt_v35 raises for a file type with no known static salt (e.g. Map)" do
  buf = bytes_of(200, 13, 7)
  buf.setbyte(1, 0x50)
  buf.setbyte(5, 0x57)
  err = assert_raise(Wolf::Error) { Wolf::Crypt.decrypt_v35(buf, "Map", Wolf::Crypt::FileType::MAP) }
  assert_true err.message.include?("map")
end

assert "Wolf::Crypt.decrypt_protected passes unprotected data through unchanged" do
  plain = "\x00hello world"
  assert_equal plain, Wolf::Crypt.decrypt_protected(plain, "x", nil)
end

assert "Wolf::Crypt.decrypt_protected decrypts a v3.5 file to a plain-file-shaped String" do
  buf = bytes_of(200, 13, 7)
  buf.setbyte(1, 0x50)
  buf.setbyte(5, 0x57)
  out = Wolf::Crypt.decrypt_protected(buf, "GameDat", Wolf::Crypt::FileType::GAME_DAT)
  assert_true out.is_a?(String)
  assert_equal 0, out.getbyte(0)
  assert_equal 67, out.bytesize
end

assert "Wolf::Crypt.decrypt_protected refuses v3.1/v3.3 by name instead of guessing" do
  v31 = ("\x00\x50\x00\x00\x00\x40" + ("\x00" * 140))
  err = assert_raise(Wolf::Error) { Wolf::Crypt.decrypt_protected(v31, "X", Wolf::Crypt::FileType::GAME_DAT) }
  assert_true err.message.include?("v3.1")

  v33 = ("\x00\x50\x00\x00\x00\x56" + ("\x00" * 140))
  err = assert_raise(Wolf::Error) { Wolf::Crypt.decrypt_protected(v33, "X", Wolf::Crypt::FileType::GAME_DAT) }
  assert_true err.message.include?("v3.3")
end

assert "Wolf::Crypt.encrypt_v35/.decrypt_v35 round-trip for every known file type" do
  header10 = [0x00, 0x50, 0x00, 0x00, 0x00, 0x57, 0x00, 0x00, 0x00, 0x00]
  junk1_10 = Array.new(10) { |i| (i * 31 + 5) & 0xFF }
  junk2_123 = Array.new(123) { |i| (i * 41 + 9) & 0xFF }
  plain_body = "Hello, WOLF RPG Editor Pro protection!"

  [
    Wolf::Crypt::FileType::GAME_DAT, Wolf::Crypt::FileType::COMMON_EVENT,
    Wolf::Crypt::FileType::DATA_BASE, Wolf::Crypt::FileType::TILE_SET_DATA
  ].each do |file_type|
    cipher = Wolf::Crypt.encrypt_v35(plain_body, file_type, header10: header10, junk1_10: junk1_10, junk2_123: junk2_123)
    assert_true Wolf::Crypt.protected?(cipher)
    decoded = Wolf::Crypt.decrypt_v35(cipher, "test", file_type)
    assert_equal Wolf::Crypt::PRO_MAGIC[file_type][:magic].pack("C*") + plain_body, decoded
  end
end

assert "Wolf::Crypt.encrypt_v35/.decrypt_v35 round-trip through the aesSize cap on a large body" do
  header10 = [0x00, 0x50, 0x00, 0x00, 0x00, 0x57, 0x00, 0x00, 0x00, 0x00]
  junk1_10 = Array.new(10) { |i| (i * 31 + 5) & 0xFF }
  junk1_10[2] = 0x7C # buffer offset 12: aes_size_for's own seed byte
  junk2_123 = Array.new(123) { |i| (i * 41 + 9) & 0xFF }
  large_body = bytes_of(5000, 7, 3)

  plain = (header10 + junk1_10 + junk2_123).pack("C*") + large_body
  assert_true Wolf::Crypt.aes_size_for(plain) < (plain.bytesize - Wolf::Crypt::AES_DATA_OFFSET)

  cipher = Wolf::Crypt.encrypt_v35(large_body, Wolf::Crypt::FileType::GAME_DAT, header10: header10, junk1_10: junk1_10, junk2_123: junk2_123)
  decoded = Wolf::Crypt.decrypt_v35(cipher, "test", Wolf::Crypt::FileType::GAME_DAT)
  assert_equal Wolf::Crypt::PRO_MAGIC[Wolf::Crypt::FileType::GAME_DAT][:magic].pack("C*") + large_body, decoded
end

assert "Wolf::Crypt.encrypt_v35/.decrypt_v35 round-trip a body past mruby's Array length cap" do
  # The regression this guards: an earlier version of decrypt_v35/
  # encrypt_v35 turned the *whole* buffer into one Array at some point,
  # which raised ArgumentError ("array size too big") against a real
  # CommonEvent.dat (see docs/adr/0095-wolf-rpg-editor-pro-protected.md) --
  # this body is bigger than mruby's MRB_ARY_LENGTH_MAX (131072) on its own.
  header10 = [0x00, 0x50, 0x00, 0x00, 0x00, 0x57, 0x00, 0x00, 0x00, 0x00]
  junk1_10 = Array.new(10) { |i| (i * 31 + 5) & 0xFF }
  junk2_123 = Array.new(123) { |i| (i * 41 + 9) & 0xFF }
  huge_body = bytes_of(200_000, 7, 3)

  cipher = Wolf::Crypt.encrypt_v35(huge_body, Wolf::Crypt::FileType::COMMON_EVENT, header10: header10, junk1_10: junk1_10, junk2_123: junk2_123)
  decoded = Wolf::Crypt.decrypt_v35(cipher, "test", Wolf::Crypt::FileType::COMMON_EVENT)
  assert_equal Wolf::Crypt::PRO_MAGIC[Wolf::Crypt::FileType::COMMON_EVENT][:magic].pack("C*") + huge_body, decoded
end
