...
module Wolf
  ...
  #     array.
  #
  # Pro-edition "protected" data (byte 1 == 0x50) is AES/ChaCha-encrypted with a
  # key that from 3.5 on is not even stored in the game; it is detected and
  # refused with a clear error rather than mis-parsed.
  UTF8_MARK = 0x55
  ...
  module Crypt
    ...
    # Pro-edition protection (3.1+; a hash-only key from 3.5) marks a file with
    # 0x50 in its second byte. Refused up front, with a message that says so.
