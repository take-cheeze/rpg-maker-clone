...
# protected files (wolf.rb's own file header):
#
...
#     the container is still plain DXA either way. So this reader does not
#     special-case protection at all: it decodes the DXA container exactly
#     the same either way, and `Wolf::Project#read`'s callers already run
#     every file through the *same* `Wolf.open_envelope`/`Crypt.
#     refuse_protected!` gate the loose-tree path always has (`data.rb`'s
#     `GameDat`/`Database`/`CommonEvents`/`Map` parsers, `Wolf.
#     open_envelope`) -- a Pro-protected packed release is refused at
#     exactly the same place, with exactly the same message, a Pro-protected
#     loose-tree one already is (see wolf.rb's own file header for why this
#     reader does not decrypt Pro-protected data at all).
#
