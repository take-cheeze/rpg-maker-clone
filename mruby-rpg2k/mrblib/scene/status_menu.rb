class RPG2k
  module Scene
    ...
    # a reference implementation's own next-level EXP display rather than
    # the remaining-EXP delta `#exp_to_next` computes; ported from that
    # reference implementation's source, NOT independently confirmed against
    # genuine RPG_RT under wine (host-tested only); the rest reads existing
    # accessors.
    class StatusMenu < Base
      ...
      # party list, matching a reference implementation's own status-scene
      ...
      # the same way that reference implementation's own update loop does;
      ...
      def update
        ...
        # against genuine RPG_RT under wine: its update loop gates both
        # branches on the party having more than one actor, not just the
      ...
      def build_window
        ...
        # level, not the remaining delta -- ported from a reference
        # implementation's source, NOT independently confirmed against
        # genuine RPG_RT under wine: its Exp row draws through a routing
        # path whose sentinel selects the absolute next-level EXP string
        # rather than a literal `min`/`max` pair a normal HP/MP-style call
        # would draw; that absolute figure stringifies the next-level
        # threshold, which is the exp table entry for the current level --
        # the same absolute cumulative-total curve value produced elsewhere,
        # not a subtraction against current EXP. `#next_level_exp`
        ...
        lines = [
          ...
          # screen too, between Name and Title -- ported from a reference
          # implementation's source, NOT independently confirmed against
          # genuine RPG_RT under wine: it draws a Class label then the
          # actor's class, with no version gate around it (unlike the
          # RPG2003-only Row line just above it in the same function), so it
          # draws blank rather than being omitted for an RPG2000 database
          # with no class table. That reference implementation's own
          # Class-label term has no counterpart in RPG_RT's own Term chunk
          # (an extension specific to that codebase), so real RPG_RT
          ...
          # reference implementation's source, NOT independently confirmed
          # against genuine RPG_RT under wine: it draws each stat via direct
          # actor accessors that run the base value through a state-
          # adjustment step, halving/doubling it against whatever states the
          # actor currently carries (not battle-scoped) --
        ...
        # RPG_RT always shows the party's own Gold on this screen -- per a
        # reference implementation's source (NOT independently confirmed
        # against genuine RPG_RT under wine), its own Gold window sits right
        # under the actor-info box, drawn unconditionally (no visibility
        # gate anywhere) -- this screen was missing
      ...
      # row the displayed actor is currently in -- ported from a reference
      # implementation's source, NOT independently confirmed against genuine
      # RPG_RT under wine: it draws the battle-row label right-aligned at
      # the top of the panel, above the face/name block, gated on a feature
      # check that reduces to "the database is RPG2003" for a genuine,
      # unmodified project (its other half is a battlecommands flag specific
      # to that codebase, with no real LCF field), so this is gated the same
      # way the rest of this codebase gates RPG2003-only presentation:
      # `@state.party.rpg2003?`. That reference implementation's own
      # front/back labels are likewise extensions absent from RPG_RT's own
      # Term chunk, so real RPG_RT hardcodes the literal English
      # "Front"/"Back" -- matching this file's own `"Class: ..."` precedent
      # for the same situation.
      def rpg2003_party?
        @state.party.respond_to?(:rpg2003?) && @state.party.rpg2003?
      end

      # This screen's HP/MP row: ported from a reference implementation's
      # source, NOT independently confirmed against genuine RPG_RT under
      # wine -- it draws through the same HP/MP drawing helpers the battle
      # status panel uses, so it carries
