...
- **LCF save-data schema validated against a real `Save01.lsd`**, generated by
  running the RPG Maker 2000 game *Nepheshel* under wine via EasyRPG Player and
  saving in-game at the town-entrance Gate. A new `scripts/lcf_save_check.rb`
  loads the parser over a real `.lsd`, lists documented vs. undocumented chunks
  and reads every documented field; `scripts/run-nepheshel-easyrpg-wine.bash`
  captures the wine/EasyRPG setup. Real data confirmed all ten documented
  sections and catalogued the still-undocumented chunks (102, 109, 112, 113,
  114, 200). See ADR 0010.

### Fixed
- **LCF reader gaps the real save exposed** (`mruby-lcf/mrblib/lcf.rb`): the
  save schema's `:bool_array` switches (chunk 32) and `:double` timestamp
  (chunk 100) had no decoder and raised `Unsupported type`; the screen-transition
  fields (`SAVE_SYSTEM` 111–116) were typed `:int` (BER) but are stored as a
  single raw byte (`0xff` = "use database value"), which `read_ber` choked on
  with a cryptic `TypeError`. Added `:bool_array`, `:double` and a scalar
  `:uint8` type, retyped the transition fields, and made `read_ber` raise a
  clear "truncated BER integer". Locked in by `mruby-lcf/test/lcf_test.rb`.
